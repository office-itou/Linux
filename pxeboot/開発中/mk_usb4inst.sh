...
#		"/mnt/hgfs/workspace/Image/linux/debian/debian-bookworm-DI-rc4-amd64-netinst.iso        ./${WORK_DIRS}/iso/net/ " \ #
		...
		"/mnt/hgfs/workspace/Image/linux/fedora/Fedora-Server-netinst-x86_64-39-1.5.iso         ./${WORK_DIRS}/iso/net/ " \
#		"/mnt/hgfs/workspace/Image/linux/fedora/Fedora-Server-netinst-x86_64-40-1.x.iso         ./${WORK_DIRS}/iso/net/ " \ #
...
#		"/mnt/hgfs/workspace/Image/linux/ubuntu/mantic-live-server-amd64.iso                    ./${WORK_DIRS}/iso/dvd/ " \ #
		...
		"/mnt/hgfs/workspace/Image/linux/fedora/Fedora-Server-dvd-x86_64-39-1.5.iso             ./${WORK_DIRS}/iso/dvd/ " \
#		"/mnt/hgfs/workspace/Image/linux/fedora/Fedora-Server-dvd-x86_64-40-1.x.iso             ./${WORK_DIRS}/iso/dvd/ " \ #
...
# --- menu.cfg list array -----------------------------------------------------
	declare -r -a MENU_LIST=(                               \
		...
		"Fedora-Server-netinst-x86_64-40-*.iso"             \
		"Fedora-Server-netinst-x86_64-39-*.iso"             \
...
#		"mantic-live-server-${ARC_TYPE}.iso"                \ #
		...
		"Fedora-Server-dvd-x86_64-40-*.iso"                 \
...
#		"debian             trixie              -                                                                                                                                           ./${WORK_DIRS}/iso/net                      -                                           -                   -           preseed_debian.cfg                              202x-xx-xx  20xx-xx-xx  -           testing             Debian_13.xx(trixie)                " \ #
		...
		"fedora             -                   https://download.fedoraproject.org/pub/fedora/linux/releases/39/Server/x86_64/iso/Fedora-Server-netinst-x86_64-39-[0-9.]*.iso               ./${WORK_DIRS}/iso/net                      -                                           -                   -           kickstart_common.cfg                            2023-11-07  2024-11-12  -           kernel_6.5          -                                   " \
#		"fedora             -                   https://download.fedoraproject.org/pub/fedora/linux/releases/40/Server/x86_64/iso/Fedora-Server-netinst-x86_64-40-[0-9.]*.iso               ./${WORK_DIRS}/iso/net                      -                                           -                   -           kickstart_common.cfg                            2024-04-16  2024-05-13  -           kernel_x.x          -                                   " \ #
...
#		"fedora             -                   https://download.fedoraproject.org/pub/fedora/linux/releases/37/Server/x86_64/iso/Fedora-Server-dvd-x86_64-37-[0-9.]*.iso                   ./${WORK_DIRS}/iso/dvd                      -                                           -                   -           kickstart_common.cfg                            2022-11-15  2023-11-14  -           kernel_6.0          -                                   " \ #
		...
		"fedora             -                   https://download.fedoraproject.org/pub/fedora/linux/releases/39/Server/x86_64/iso/Fedora-Server-dvd-x86_64-39-[0-9.]*.iso                   ./${WORK_DIRS}/iso/dvd                      -                                           -                   -           kickstart_common.cfg                            2023-11-07  2024-11-12  -           kernel_6.5          -                                   " \
#		"fedora             -                   https://download.fedoraproject.org/pub/fedora/linux/releases/40/Server/x86_64/iso/Fedora-Server-dvd-x86_64-40-[0-9.]*.iso                   ./${WORK_DIRS}/iso/dvd                      -                                           -                   -           kickstart_common.cfg                            2024-04-16  2025-05-13  -           kernel_x.x          -                                   " \ #
...
#		fi
		...
		LINXIMG_ARRAY=($(find "./${WORK_DIRS}/deb/${DIR_CODE}" "./${WORK_DIRS}/deb/${DIR_DIST}" \( ${DIR_WORK} \) \( -type f -o -type l \)))
		if [[ ${#LINXIMG_ARRAY[@]} -le 0 ]]; then
			#idx: value
			#  0: distribution
			#  1: codename
			#  2: download URL
			...
			for I in "${!PACKAGE_FILE[@]}"
			do
				PACKAGE_ARRAY=(${PACKAGE_FILE[${I}]})
				if [[ "${DIR_CODE}" != "${PACKAGE_ARRAY[0]}.${PACKAGE_ARRAY[1]}" ]]; then
					continue
				fi
				case "${PACKAGE_ARRAY[2]##*/}" in
					linux-image-* )
						WEB_PAGE="${PACKAGE_ARRAY[2]%/*}"
						set +e
						WEB_INFO=("$(curl --location --http1.1 --no-progress-bar --remote-time --show-error --silent --fail --retry-max-time 3 --retry 3 "${WEB_PAGE}" 2> /dev/null)")
						RET_CD=$?
						set -e
						if [[ ${RET_CD} -eq 6 ]] || [[ ${RET_CD} -eq 18 ]] || [[ ${RET_CD} -eq 22 ]] || [[ ${RET_CD} -eq 28 ]] || [[ ${#WEB_INFO[@]} -le 0 ]]; then
							FILE_INFO=""
						else
							FILE_NAME="linux-image-${DIR_KVER}_.*_${ARC_TYPE}\.deb"
							FILE_INFO=($(echo "${WEB_INFO[@]}" | sed -e 's/\'$'\r//g' | LANG=C sed -n -e "/${FILE_NAME}/ s/^.*<a href=.*> *\(${FILE_NAME}\) *<\/a.*> *\([0-9a-zA-Z]*-[0-9a-zA-Z]*-[0-9a-zA-Z]*\) *\([0-9]*:[0-9]*\).*$/\1 \2 \3/p" | sort -r | head -n 1))
							funcCurl --location --progress-bar --remote-name --remote-time --show-error --fail --retry-max-time 3 --retry 3 --create-dirs --output-dir "${PACKAGE_ARRAY[3]}" "${WEB_PAGE}/${FILE_INFO[0]}"
						fi
						break
						;;
					* )
