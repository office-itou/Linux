...
##	2023/11/10 000.0000 J.Itou         処理見直し
##	2023/11/15 000.0000 J.Itou         処理見直し
...
#
	...
	if [ -n "${DIR_ZONE:-}" ]; then
		...
		if [ ! -d ${DIR_ZONE}/master ]; then
			...
			DIR_MAST="${DIR_ZONE}/master"
			mkdir -p ${DIR_MAST}
			chown ${DNS_USER}:${DNS_GRUP} ${DIR_MAST}
		fi
		if [ ! -d ${DIR_ZONE}/slave ]; then
			fncPrint "--- mkdir slaves $(fncString ${COL_SIZE} '-')"
			DIR_SLAV="${DIR_ZONE}/slaves"
			mkdir -p ${DIR_SLAV}
			chown ${DNS_USER}:${DNS_GRUP} ${DIR_SLAV}
		fi
		...
		do
			cat <<- _EOT_ | sed -e 's/^ //g' > ${DIR_MAST}/db.${FIL_NAME}
				\$TTL 1H																; 1 hour
...
_EOT_
#			chmod 640 ${DIR_MAST}/db.${FIL_NAME}
			chown ${DNS_USER}:${DNS_GRUP} ${DIR_MAST}/db.${FIL_NAME}
		done
		#----------------------------------------------------------------------
		cat <<- _EOT_ | sed -e 's/^ //g' >> ${DIR_MAST}/db.${WGP_NAME}
			${SVR_NAME}								IN		A		${IP4_ADDR[0]}
			 										IN		AAAA	${LNK_ADDR[0]}
			 										IN		AAAA	${IP6_ADDR[0]}
_EOT_
		#----------------------------------------------------------------------
		cat <<- _EOT_ | sed -e 's/^ //g' >> ${DIR_MAST}/db.${IP4_RADR[0]}.in-addr.arpa
			 										IN		PTR		${WGP_NAME}.
			 										IN		A		${IP4_MASK}
			${IP4_LADR[0]}										IN		PTR		${SVR_NAME}.${WGP_NAME}.
_EOT_
		#----------------------------------------------------------------------
		cat <<- _EOT_ | sed -e 's/^ //g' >> ${DIR_MAST}/db.${LNK_RADU[0]}.ip6.arpa
			 										IN		PTR		${WGP_NAME}.
			${LNK_RADL[0]}			IN		PTR		${SVR_NAME}.${WGP_NAME}.
_EOT_
		#----------------------------------------------------------------------
		cat <<- _EOT_ | sed -e 's/^ //g' >> ${DIR_MAST}/db.${IP6_RADU[0]}.ip6.arpa
			 										IN		PTR		${WGP_NAME}.
...
_EOT_
		...
		if [ "${IP4_DHCP[0]}" = "auto" ]; then
			...
			sed -i.orig ${DIR_MAST}/db.${WGP_NAME}                 -e "/^${SVR_NAME}.*${IP4_ADDR[0]}$/d"
			sed -i.orig ${DIR_MAST}/db.${IP4_RADR[0]}.in-addr.arpa -e "/^${IP4_LADR[0]}.*${SVR_NAME}\.${WGP_NAME}\.$/d"
		fi
		# --- named.conf ------------------------------------------------------
...
#	fi
	# Install bind9 ***********************************************************
#	if [ -f ${DIR_MAST}/db.${WGP_NAME}                 ]; then
#		fncPrint "--- cat ${DIR_MAST}/db.${WGP_NAME} $(fncString ${COL_SIZE} '-')"
#		expand -t 4 ${DIR_MAST}/db.${WGP_NAME}
#	fi
#	if [ -f ${DIR_MAST}/db.${IP4_RADR[0]}.in-addr.arpa ]; then
#		fncPrint "--- cat ${DIR_MAST}/db.${IP4_RADR[0]}.in-addr.arpa $(fncString ${COL_SIZE} '-')"
#		expand -t 4 ${DIR_MAST}/db.${IP4_RADR[0]}.in-addr.arpa
#	fi
#	if [ -f ${DIR_MAST}/db.${IP6_RADU[0]}.ip6.arpa     ]; then
#		fncPrint "--- cat ${DIR_MAST}/db.${IP6_RADU[0]}.ip6.arpa $(fncString ${COL_SIZE} '-')"
#		expand -t 4 ${DIR_MAST}/db.${IP6_RADU[0]}.ip6.arpa
#	fi
#	if [ -f ${DIR_MAST}/db.${LNK_RADU[0]}.ip6.arpa     ]; then
#		fncPrint "--- cat ${DIR_MAST}/db.${LNK_RADU[0]}.ip6.arpa $(fncString ${COL_SIZE} '-')"
#		expand -t 4 ${DIR_MAST}/db.${LNK_RADU[0]}.ip6.arpa
#	fi
	if [ -n "${DIR_ZONE:-}" ]; then
		if [ -d ${DIR_MAST} ]; then
			for FILE in `find ${DIR_MAST} ${DIR_SLAV} -type f -print`
			do
...
#	set +e
	...
	[ -f ${DIR_MAST}/db.${WGP_NAME}.orig                 ] && { mv ${DIR_MAST}/db.${WGP_NAME}.orig                 ${DIR_MAST}/db.${WGP_NAME}                ; }
	[ -f ${DIR_MAST}/db.${IP4_RADR[0]}.in-addr.arpa.orig ] && { mv ${DIR_MAST}/db.${IP4_RADR[0]}.in-addr.arpa.orig ${DIR_MAST}/db.${IP4_RADR[0]}.in-addr.arpa; }
