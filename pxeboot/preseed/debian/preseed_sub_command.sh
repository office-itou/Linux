...
#	NIC_MADR="$(LANG=C ip address show dev "${NIC_NAME}" 2> /dev/null | sed -n -e '/link\/ether/ s%.*link/ether \([[:graph:]]\+\) .*$%\1%p')"
	...
	NIC_NAME="$(LANG=C ip -oneline link 2> /dev/null | sed -n -e '/2:/ s%.*2: \([[:graph:]]\+\): .*$%\1%p')"
	NIC_IPV4="$(LANG=C ip -oneline -4 address show dev "${NIC_NAME}" 2> /dev/null | sed -n -e '/inet/ s%.*inet \([[:graph:]]\+\)/.*$%\1%p')"
	NIC_BIT4="$(LANG=C ip -oneline -4 address show dev "${NIC_NAME}" 2> /dev/null | sed -n -e '/inet/ s%.*inet .*/\([[:graph:]]\+\) .*$%\1%p')"
	NIC_MASK="$(funcIPv4GetNetmask "${NIC_BIT4}")"
	NIC_GATE="$(LANG=C ip -oneline -4 route show dev "${NIC_NAME}" 2> /dev/null | sed -n -e '/default/ s%.*via \([[:graph:]]\+\) .*$%\1%p')"
	NIC_DNS4="$(LANG=C sed -n -e '/nameserver/ s%nameserver \([[:graph:]]\+\).*$%\1%p' /target/etc/resolv.conf)"
	NIC_WGRP="$(LANG=C sed -n -e '/search/ s%search \([[:graph:]]\+\).*$%\1%p' /target/etc/resolv.conf)"
	NIC_MADR="$(LANG=C ip -oneline link show dev "${NIC_NAME}" 2> /dev/null | sed -n -e '/link\/ether/ s%.*link/ether \([[:graph:]]\+\) .*$%\1%p')"
	CON_NAME="ethernet_$(echo "${NIC_MADR}" | sed -n -e 's/://gp')_cable"
	echo "${PROG_NAME}: HOST_FQDN=${HOST_FQDN}"
