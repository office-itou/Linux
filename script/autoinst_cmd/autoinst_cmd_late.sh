...
fnSetup_clamav() {
	...
	if command -v freshclam > /dev/null 2>&1; then
		if freshclam; then
			fnMsgout "${_PROG_NAME:-}" "success" "freshclam"
		else
			fnMsgout "${_PROG_NAME:-}" "failed" "freshclam ($?)"
		fi
		if chown --recursive clamav:clamav /var/log/clamav/; then
			fnMsgout "${_PROG_NAME:-}" "success" "chown"
		else
			fnMsgout "${_PROG_NAME:-}" "failed" "chown ($?)"
		fi
