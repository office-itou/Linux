...
# *** global section **********************************************************
	...
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnComm_global_common.sh		# global variables (for basic)
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnComm_global_variables.sh		# global variables (for user parameter)

# *** function section (common functions) *************************************

	# shellcheck source=/dev/null
#	source "${_SHEL_COMN:?}"/fnColorcode.sh					# color code
	# shellcheck source=/dev/null
...
#	source "${_SHEL_COMD:?}"/fnComm_list_mdia_Put.sh		# put media information data
	...
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnComm_hostname.sh				# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnComm_nic_name.sh				# 

	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_autoexec.sh		# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_menu.sh			# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_booting.sh			# 
	# shellcheck source=/dev/null
#	source "${_SHEL_TOPS:?}"/fnMake_ipxe_live.sh			# 
	# shellcheck source=/dev/null
#	source "${_SHEL_TOPS:?}"/fnMake_ipxe_custom_live.sh		# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_debian.sh			# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_ubuntu.sh			# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_fedora.sh			# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_centos.sh			# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_almalinux.sh		# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_rockylinux.sh		# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_miraclelinux.sh	# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_opensuse.sh		# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe_windows.sh			# 
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_ipxe.sh					# 

# *** main section ************************************************************

	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_init_mk_ipxe_menu.sh	# initialize
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_help_mk_ipxe_menu.sh	# help
	# shellcheck source=/dev/null
	source "${_SHEL_TOPS:?}"/fnMake_main_mk_ipxe_menu.sh	# main routine
