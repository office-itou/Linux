...
#   info_conf.load()
#   info_conf.dump()
    info_conf.markdown(Path('.') / 'Readme_Configuration.md', f"Configuration data({Path(info_conf.get('PATH_CONF')).name})")
#   a = info_conf.conv2data(data)
#   a = info_conf.conv2variable(data)
#   a = info_conf.get(key)
    # -------------------------------------------------------------------------
#   info_dist.dump()
    info_dist.markdown(Path('.') / 'Readme_Distribution.md' , f"Distribution data({Path(info_conf.get('PATH_DIST')).name})")
#   info_dist.load(info_conf.get('PATH_DIST') + '.json')
    info_dist.save(info_conf.get('PATH_DIST') + '.test.json')
    # -------------------------------------------------------------------------
#   info_mdia.dump()
    info_mdia.markdown(Path('.') / 'Readme_Media.md'        , f"Media data({Path(info_conf.get('PATH_MDIA')).name})")
