...
def version_newer(target_urls: list[str]) -> list[str]:
    _DISTRIBUTION_LIST = [
        "debian",
        "ubuntu",
        "fedora",
    ...
    for d in _CODE_LIST:
        _code_name.append(
            _CodeName(
                ...
                version=d["version"],
                codename=d["codename"],
                codename_id=rf"/{d['codename'].split()[0].lower()}/",
            )
        )
    ...
    # -------------------------------------------------------------------------
    def _get_distribution(url: str) -> str:
        for s in _DISTRIBUTION_LIST:
            if s in _url:
                return s
        return "_generic_os"

    def _get_flavor(_filename_lower: str) -> str:
        for s in _FLAVOR_LIST:
            if s in _filename_lower:
                return s
        return "_generic"

    def _get_development(_filename_lower: str) -> str:
    ...
    def _get_version(
        ...
        if any(c.isdigit() for c in filename_lower):
            match = version_pattern.search(filename_lower)
            if match:
                return match.group(2)
        for d in _code_name:
            if (
                d.codename_id in url_lower
                or rf"{d.codename_id[:-1]}-updates/" in url_lower
            ):
                return d.version
        return _get_development(filename_lower)

    def _sort_key_version(item):
        _pattern_str = rf"^([a-zA-Z0-9_-]+?)-([0-9.]+|{'|'.join(_DEVELOPMENT_VERSION)})"
        v_str = item.version
        base_match = re.match(_pattern_str, v_str)
        if base_match:
    ...
    def _sort_key_distribution(item):
        v_str = item.distribution
    ...
    _resolved_urls = sorted(set(target_urls), reverse=True)
    ...
    for _url in _resolved_urls:
        ...
        _target_datas.append(
            _SortData(
                url=_url,
                distribution=_get_distribution(_url_lower),
                filename=_filename,
                version=_get_version(_url_lower, _filename_lower, _version_pattern),
                flavor=_get_flavor(_filename_lower),
            )
        )
    _step1 = sorted(_target_datas, key=_sort_key_version, reverse=True)
    _sorted_urls = sorted(_step1, key=_sort_key_distribution, reverse=False)
    # -------------------------------------------------------------------------
    _result_datas = _sorted_urls
    # -------------------------------------------------------------------------
...
for d in sorted_urls:
    print(f"{d.filename:<50},{d.version},{d.url}")
