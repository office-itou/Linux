...
def message_date(function_name: str, mode: str, message_color: str, date_time: str):
    ...
    eprint(
        f"{color.reset}{message_color}{function_name:<{colsize_func}}|{mode:^{colsize_mode}}|{message}{color.reset}",
        infosystem.columns,
    )
...
def message_start(function_name: str):
    date_time = datetime.now(tz=datetime.UTC).strftime("%Y/%m/%d %H:%M:%S")
...
def message_info(function_name: str, message: str):
    text_prog = omit_middle(f"{infosystem.program_name}({function_name})", colsize_func)
    eprint(
        f"{color.reset}{color.br_green}{text_prog:<{colsize_func}}|{'info':^{colsize_mode}}|{message}{color.reset}"
    )
...
def message_warn(function_name: str, message: str):
    text_prog = omit_middle(f"{infosystem.program_name}({function_name})", colsize_func)
    eprint(
        f"{color.reset}{color.br_yellow}{text_prog:<{colsize_func}}|{'warn':^{colsize_mode}}|{message}{color.reset}"
    )
...
def message_alert(function_name: str, message: str):
    text_prog = omit_middle(f"{infosystem.program_name}({function_name})", colsize_func)
    eprint(
        f"{color.reset}{color.br_red}{text_prog:<{colsize_func}}|{'alert':^{colsize_mode}}|{message}{color.reset}"
    )
