...
#		debian-live-*                   ) __WORK="${__WORK:+"${__WORK} "}utc=yes locales=ja_JP.UTF-8 timezone=Asia/Tokyo key-model=pc105 key-layouts=jp key-variants=OADG109A";;
	...
	case "${__MDIA[$((_OSET_MDIA+0))]}" in
		live) __WORK="${__WORK:+"${__WORK} "}utc=yes locales=ja_JP.UTF-8 timezone=Asia/Tokyo key-model=pc105 key-layouts=jp key-variants=OADG109A";;
		*) ;;
	esac
	__BOPT+=("${__WORK:-}")
	# --- 3: network ----------------------------------------------------------
...
# -----------------------------------------------------------------------------
# descript: make linux cnetinst section for ipxe menu
#   input :     $@     : media info data
#   output:   stdout   : output
#   return:            : unused
function fnMk_pxeboot_ipxe_linux_cnetinst() {
	declare -a    __MDIA=("${@:-}")
	declare -a    __BOPT=()
	declare       __NICS=""
	declare       __HOST=""
	...
	cat <<- _EOT_ | sed -e '/^ [^ ]\+/ s/^ *//g' -e 's/^ \+$//g' || true
		...
		set networks ${__BOPT[2]:-}
		set otheropt ${__BOPT[@]:3} --- quiet${_MENU_MODE:+" vga=${_MENU_MODE}"}
		#set nfsoptns netboot=nfs nfsroot=\${srvraddr}:/srv/exports/nfs/imgs/ubuntu-desktop-26.04
		set nbdfiles root=nbd:\${srvraddr}:debian-testing-amd64-DVD-1.iso
		#set nbdfiles root=nbd:\${srvraddr}:ubuntu-26.04-desktop-amd64.iso
		set otheropt vga=normal nomodeset \${nfsoptns} \${nbdfiles}
		set consoles dummy_console=tty0 dummy_console=ttyS0,9600
		set sulogins SYSTEMD_SULOGIN_FORCE=1 dummy_init=/sbin/sulogin
		set debugopt \${consoles} \${sulogins}
		form                                    Configure Autoinstall Options
