	...
	if [[ "$#" -gt 0 ]] && [[ -n "${*:-}" ]]; then
		...
		printf "%s\n" "${@:-}" | awk '
		{
			...
			for (i in _arry) {
				...
				sub("^-+$", "", _list[17])
				sub("^-+$", "", _list[27])
				_list[18]=substr(_list[18], 1, 10)
				_list[10]=substr(_list[10], 1, 10)
				_list[27]=substr(_list[27], 1, 44)
				sub("^-+$", "\033[m\033[90m----------\033[m", _list[18])
				sub("^-+$", "\033[m\033[90m----------\033[m", _list[10])
				sub("^$", "\033[m\033[37m"_list[6]"\033[m", _list[27])
				printf("\033[m%c%2d:%-48.48s:%-10s:%-10s:%-44s%c\033[m\n", "#", _list[2], _list[17], _list[18], _list[10], _list[27], "#")
			}
		}'
		printf "%c${__FMTG}%c\n" "#" "${_TEXT_GAP2}" "#"
	fi
