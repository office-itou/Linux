...
#	readonly _PROG_PROC="${_PROG_NAME}.$$"
	...
	_ISOS_FILE=""						# iso file name
	_SEED_FILE=""						# preseed file name
	# --- target --------------------------------------------------------------
...
fnGet_param_network() {
	...
	if [ -z "${_NICS_BIT4}" ]; then
		_NICS_BIT4="$(fnIPv4GetNetmask "${_NICS_MASK:-"255.255.255.0"}")"
	else
		_NICS_MASK="$(fnIPv4GetNetmask "${_NICS_BIT4:-"24"}")"
	fi
