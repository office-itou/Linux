...
function funcTrap() {
	...
	declare -r -a LIST_DIRS=(                                                                                           \
		...
		"${DIRS_CONF}"/{_keyring,_template,autoyast,kickstart,nocloud,preseed,autoinst,windows}                         \
		"${DIRS_KEYS}"                                                                                                  \
		"${DIRS_TMPL}"                                                                                                  \
		"${DIRS_IMGS}"                                                                                                  \
		"${DIRS_ISOS}"                                                                                                  \
...
function funcCreate_copy_iso2hdd() {
	...
	declare       _DIRS_VLNZ=""								# kernel image directory
	declare       _FILE_IRAM=""								# initrd path
	declare       _FILE_VLNZ=""								# kernel path
	declare -a    _RSYC_OPTN=("--recursive" "--links" "--perms" "--times" "--group" "--owner" "--devices" "--specials" "--hard-links" "--acls" "--xattrs" "--human-readable" "--update" "--delete")
	declare -r    _DEST_DIRS="${DIRS_IMGS}/${_TGET_LINE[1]}"
	declare -r    _BOOT_DIRS="${DIRS_LOAD}/${_TGET_LINE[1]}"
	# -------------------------------------------------------------------------
...
# ----- create menu for preseed -----------------------------------------------
function funcCreate_menu_preseed() {
	declare -r -a _TGET_LINE=("$@")
	declare       _BOOT_OPTN=""
...
function funcCreate_menu_nocloud() {
	declare -r -a _TGET_LINE=("$@")
	declare       _BOOT_OPTN=""
	declare -r    _HOST_NAME="sv-${_TGET_LINE[1]%%-*}"
...
function funcCreate_menu_kickstart() {
	declare -r -a _TGET_LINE=("$@")
	declare       _BOOT_OPTN=""
	declare -r    _HOST_NAME="sv-${_TGET_LINE[1]%%-*}"
...
function funcCreate_menu_autoyast() {
...
#	declare       _NWRK_MANE=""
#	declare       _NWRK_CONF=""
	declare       _LANG_CONF="language=ja_JP"
	declare       _ISOS_FILE="install=http://\${srvraddr}/imgs/${_TGET_LINE[1]}"
#	declare       _WEBS_FILE="install=https://download.opensuse.org/distribution/leap/${_TGET_LINE[1]##*-}/repo/oss/"
	declare       _WEBS_FILE="install=https://ftp.riken.jp/Linux/opensuse/distribution/leap/${_TGET_LINE[1]##*-}/repo/oss/"
#	funcPrintf "%20.20s: %s" "create" "boot options for kickstart"
	# --- boot option ---------------------------------------------------------
	_BOOT_OPTN=""
	# --- autoinst ------------------------------------------------------------
	...
	if [[ "${_TGET_LINE[9]##*/}" = "-" ]]; then
		...
		_BOOT_OPTN+="${_BOOT_OPTN:+" "}hostname=\${hostname} ifcfg=\${ethrname}=\${ipv4addr}/\${ipv4mask},\${ipv4gway},\${ipv4nsvr},${WGRP_NAME}"
		case "${_TGET_LINE[1]}" in
			opensuse-*-15* ) _WORK_ETHR="eth0";;
			*              ) ;;
		esac
		case "${_TGET_LINE[1]}" in
#			opensuse-tumbleweed-netinst ) _ISOS_FILE="install=https://download.opensuse.org/tumbleweed/repo/oss/";;
			opensuse-tumbleweed-netinst ) _ISOS_FILE="install=https://ftp.riken.jp/Linux/opensuse/tumbleweed/repo/oss/";;
			opensuse-*-netinst-*        ) _ISOS_FILE="${_WEBS_FILE}";;
			*                           ) ;;
		esac
	fi
	# --- language ------------------------------------------------------------
...
_EOT_
		...
		IFS= cat <<- _EOT_ | sed -e '/^ [^ ]\+/ s/^ *//g' -e 's/^ \+$//g'
			${_BOOT_OPTN}
			set hostname ${_HOST_NAME:-"sv-${_TGET_LINE[1]%%-*}"}${WGRP_NAME:+".${WGRP_NAME}"}
			set ethrname ${_WORK_ETHR:-"${ETHR_NAME:-ens160}"}
			set ipv4addr ${IPV4_ADDR:-}
			set ipv4mask ${IPV4_CIDR}
			set ipv4gway ${IPV4_GWAY:-}
			set ipv4nsvr ${IPV4_NSVR:-}
			set srvraddr ${SRVR_ADDR:?}
			isset \${next-server} && set srvraddr \${next-server} ||
				...
				windows-* )
					_MENU_TEXT="$(
						cat <<- _EOT_ | sed -e '/^ [^ ]\+/ s/^ *//g' -e 's/^ \+$//g' | sed -e ':l; N; s/\n/\\n/; b l;'
							...
							initrd -n install.cmd \${cfgaddr}/inst_w${_TGET_LINE[1]##*-}.cmd  install.cmd  || goto error
...
#	declare -r    _MENU_AUTO="${DIRS_TFTP}/boot/grub/autoinst.cfg"
	...
	for I in "${!DATA_LIST[@]}"
	do
		read -r -a _TGET_LINE < <(echo "${DATA_LIST[I]}")
		case "${_TGET_LINE[0]}" in
			m )								# menu
...
#	trap funcTrap EXIT
	...
	if [[ -z "${PROG_PARM[*]}" ]]; then
		...
		funcPrintf "  create pxeboot menu files"
		funcPrintf "    --create"
		funcPrintf "  create pxeboot menu files without copying iso image"
		funcPrintf "    --update"
