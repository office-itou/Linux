...
function fnGet_webdata(_retn, _urls,  i, j, _list, _line, _code, _leng, _lmod, _ptrn, _dirs, _file, _rear) {
	_leng=""
	_lmod=""
	# --- set pattern part ----------------------------------------------------
	_ptrn=""
...
#	match(_urls, "[^/]*\\[.*\\].*$")
#	if (RSTART > 0) {
#		_ptrn=substr(_urls, RSTART, RLENGTH)
#	}
#	sub("/.*$", "", _ptrn)
#	_dirs=_urls
#	match(_urls, "/[^/]*\\[.*\\].*$")
...
#		_comd="wget --tries=3 --timeout=60 --quiet --spider --server-response --output-document=- "_urls" 2>&1"
#	} else {
#		_urls=substr(_urls, 1, RSTART-1)
#		_comd="wget --tries=3 --timeout=60 --quiet --server-response --output-document=- "_urls" 2>&1"
#	}
#	delete _retn
#	fnGet_webdata(_comd, _retn)
